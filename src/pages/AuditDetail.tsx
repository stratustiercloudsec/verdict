...
const AuditDetail: React.FC = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Verdict_Audit_${data.projectName?.replace(/\s+/g, '_')}.pdf`);
  ...
  if (loading) return <div className="p-10 text-center animate-pulse italic text-meta-3 font-black uppercase">Assembling Creative Intelligence...</div>;
  if (error) return <div className="p-10 text-center text-red-500 font-bold">{error}</div>;

  return (
    <>
      <Breadcrumb pageName={`Audit: ${data.projectName}`} />
      ...
      >
        ...
        <div className="mb-10 text-center">
          <h1 className="text-5xl font-black text-black dark:text-white uppercase mb-2 tracking-tighter">{data.projectName}</h1>
          <div className="h-1.5 w-32 bg-meta-3 mx-auto"></div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-4 mb-10">
          ...
          <div className="rounded-lg border-2 border-stroke bg-gray-50 p-6 text-center dark:border-strokedark dark:bg-meta-4 shadow-sm flex flex-col justify-center">
            <h4 className="text-[10px] font-black uppercase text-gray-500 mb-2 tracking-widest">Lead Writer</h4>
            <span className="text-xs font-black text-black dark:text-white uppercase leading-tight">{data.writer || data.author || "Unknown"}</span>
          </div>
          <div className="rounded-lg border-2 border-stroke bg-gray-50 p-6 text-center dark:border-strokedark dark:bg-meta-4 shadow-sm flex flex-col justify-center">
            <h4 className="text-[10px] font-black uppercase text-gray-500 mb-2 tracking-widest">Character Count</h4>
            <span className="text-4xl font-black text-meta-3 uppercase">{data.character_count || 0}</span>
          </div>
          <div className="rounded-lg border-2 border-stroke bg-gray-50 p-6 text-center dark:border-strokedark dark:bg-meta-4 shadow-sm flex flex-col justify-center">
            <h4 className="text-[10px] font-black uppercase text-gray-500 mb-2 tracking-widest">Primary Tone</h4>
            <span className="text-[11px] font-black text-meta-3 uppercase leading-tight">{data.tone || "N/A"}</span>
          </div>
        </div>

        {/* Section 1.0: Executive Analysis */}
        <div className="mb-12">
          <h3 className="mb-6 text-xs font-black uppercase tracking-[0.2em] border-b-2 border-meta-3 pb-2 text-black dark:text-white">
            1.0 Executive Analysis Summary & Synopsis
          </h3>
        ...
        {/* Section 2.0: Narrative Structure */}
        <div className="space-y-12">
          <div>
            <h3 className="mb-6 text-xs font-black uppercase tracking-[0.2em] border-b-2 border-meta-3 pb-2 text-black dark:text-white">
              2.0 Narrative Structure
            </h3>
            <div className="mb-8 bg-black text-white p-10 rounded-md shadow-xl border-l-8 border-meta-3">
            ...
            <div className="pt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {data.characters.map((char: any, idx: number) => (
                  <div key={idx} className="border-l-4 border-meta-3 pl-6 py-2 bg-gray-50 dark:bg-meta-4 rounded-r-md">
                    <p className="text-sm font-black uppercase text-black dark:text-white mb-1">{char.name}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 leading-normal italic">{char.description}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Section 3.0: Strategic Intelligence */}
          <div>
            <h3 className="mb-6 text-xs font-black uppercase tracking-[0.2em] border-b-2 border-meta-3 pb-2 text-black dark:text-white">
            ...
            <div className="bg-gray-100 dark:bg-meta-4 p-10 rounded-md border-l-8 border-black shadow-sm">
                ...
                <p className="text-black dark:text-white text-md leading-relaxed font-semibold italic">{data.comments || "Critique finalizing..."}</p>
            </div>
          </div>
        </div>
