...
const CoverageReport: React.FC = () => {
        ...
        interval = setInterval(async () => {
          try {
            const res = await fetch(
              `https://jdig9yqazd.execute-api.us-east-1.amazonaws.com/prod/get-audit?auditId=${currentAuditId}`
            );
            ...
            if (payload.status === 'COMPLETED') {
              ...
              // Auto-navigate to audit detail page after brief completion flash
              setTimeout(() => navigate(`/audit-detail/${currentAuditId}`), 1500);
            } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uploadUrlRes = await fetch(
        'https://jdig9yqazd.execute-api.us-east-1.amazonaws.com/prod/get-upload-url',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            fileName: `${masterId}-${selectedFile.name}`,
            reportType,
          }),
        }
      );
      ...
      const contentType = selectedFile.name.endsWith('.docx')
      ...
      // Step 3 — Kick off coverage analysis
      const analysisRes = await fetch(
        'https://jdig9yqazd.execute-api.us-east-1.amazonaws.com/prod/coverage',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (analysisRes.ok) {
        // ✅ Register job with notification system — bell will track QUEUED → COMPLETED
  ...
  const cardClass =
    'rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark mb-6';
      ...
      {loading && (
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/80 backdrop-blur-md">
          ...
          <p className="mt-2 text-white/70 italic font-serif">
            Deploying Verdict AI analysis layers...
          </p>
        </div>
      )}
          ...
          <div className="mb-6 rounded-sm border border-stroke bg-white p-8 shadow-default dark:border-strokedark dark:bg-boxdark text-center">
            <h3
              className={`text-xl font-black uppercase tracking-widest ${
                status === 'complete' ? 'text-success' : 'text-meta-3'
              }`}
            >
            ...
            <div className="mt-6 h-4 w-full rounded-full bg-gray-2 dark:bg-meta-4">
              <div
                className={`h-full rounded-full transition-all duration-1000 ${
                  status === 'complete' ? 'bg-success' : 'bg-meta-3 animate-pulse'
                }`}
          ...
          <div className="p-6.5 flex flex-col gap-6">
            <div>
              ...
              <input
                ...
                onChange={(e) => setUserDetails({ ...userDetails, projectName: e.target.value })}
                className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 outline-none focus:border-meta-3 dark:border-form-strokedark"
              />
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 outline-none focus:border-meta-3"
                />
              </div>
              <div>
                <label className="mb-3 block text-black dark:text-white font-bold uppercase text-xs tracking-widest flex items-center">
            ...
            <div>
              ...
              <div className="flex gap-8 p-4 rounded-lg bg-gray-2 dark:bg-meta-4">
                {['Full Script', 'Treatment'].map((t) => (
                  <label
                    key={t}
                    className="flex items-center gap-3 text-xs font-black uppercase cursor-pointer text-black dark:text-white hover:text-meta-3 transition-colors"
                  >
                    <input
                      type="radio"
                      checked={reportType === t}
                      onChange={() => setReportType(t)}
                      className="form-radio text-meta-3 h-5 w-5"
                    />
                    {t}
                  </label>
                ))}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full cursor-pointer rounded-lg border-[1.5px] border-stroke bg-transparent font-medium outline-none transition file:mr-5 file:py-3 file:px-5 file:border-0 file:bg-meta-3 file:text-white file:hover:bg-opacity-90 dark:border-strokedark"
                />
